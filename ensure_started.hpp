...
namespace execution{
	namespace ensure_started_detail{
		...
		struct ensure_started_env{
			template<typename Self> requires std::same_as<std::remove_cvref_t<Self>, ensure_started_env>
		...
		struct receiver_impl{
			...
			[[nodiscard]] friend ensure_started_env tag_invoke(get_env_t, receiver_impl r) noexcept{
				return ensure_started_env{ r.shared_state_get_stop_token() };
			}
		};

		template<typename Sh>
		using ref_t = std::shared_ptr<Sh>; // it's confirmed that an instance of ensure_started_sender can not be used by more than one thread.
		template<typename Sh>
		...
		template<sender<ensure_started_env> S>
		struct sender_impl{
		private:
			...
			using value_types_list = value_types_of_t<S, ensure_started_env, value_type_list, type_list>;
			using error_types_list = include_or_add_t<error_types_of_t<S, ensure_started_env, error_type_list>, type_list<set_error_t, std::exception_ptr>>;
		...
		template<sender<ensure_started_env> S>
		using ensure_started_sender = sender_impl<std::remove_cvref_t<S>>;
	...
	struct ensure_started_t{
		...
		template<sender<ensure_started_detail::ensure_started_env> S> requires tag_invocable_with_completion_scheduler<ensure_started_t, set_value_t, S&&> && sender<tag_invoke_result_t<ensure_started_t, completion_scheduler_for<S&&, set_value_t>, S&&>>
        [[nodiscard]] auto operator()(S&& s) const noexcept(nothrow_tag_invocable<ensure_started_t, completion_scheduler_for<S&&, set_value_t>, S&&>)->tag_invoke_result_t<ensure_started_t, completion_scheduler_for<S&&, set_value_t>, S&&>{
			auto cs = get_completion_scheduler<set_value_t>(s); // guarantee the order of evaluation
            return tag_invoke(ensure_started_t{}, std::move(cs), std::forward<S>(s));
        }

        template<sender<ensure_started_detail::ensure_started_env> S> requires tag_invocable<ensure_started_t, S&&> && (!tag_invocable_with_completion_scheduler<ensure_started_t, set_value_t, S&&>) && sender<tag_invoke_result_t<ensure_started_t, S&&>>
        [[nodiscard]] auto operator()(S&& s) const noexcept(nothrow_tag_invocable<ensure_started_t, S&&>)->tag_invoke_result_t<ensure_started_t, S&&>{
            return tag_invoke(ensure_started_t{}, std::forward<S>(s));
        }

        template<sender<ensure_started_detail::ensure_started_env> S> requires (!tag_invocable<ensure_started_t, S&&>) && (!tag_invocable_with_completion_scheduler<ensure_started_t, set_value_t, S&&>)
        [[nodiscard]] auto operator()(S&& s) const noexcept(std::is_nothrow_constructible_v<std::remove_cvref_t<S>, S&&>){
            return ensure_started_detail::ensure_started_sender<S>{ std::forward<S>(s) };
        }
	};
	inline constexpr ensure_started_t ensure_started{};
	template<sender<ensure_started_detail::ensure_started_env> S>
    using ensure_started_result_t = decltype(ensure_started(std::declval<S>()));
	template<typename S>
	concept nothrow_ensure_started_invocable = sender<S, ensure_started_detail::ensure_started_env> && noexcept(ensure_started(std::declval<S>()));

	namespace ensure_started_detail{
        struct ensure_started_closure_t : public sender_adaptor_closure<ensure_started_closure_t>{
			...
			[[nodiscard]] auto get_tag() const noexcept{
				return ensure_started_t{};
			}
        };
	}
	[[nodiscard]] inline auto ensure_started_t::operator()() const noexcept{
		return ensure_started_detail::ensure_started_closure_t{};
	}
}